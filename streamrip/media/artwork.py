...
async def download_artwork(
    ...
    if save_artwork:
        ...
        if config.saved_max_width > 0:
            # ⚡ Bolt: Offload CPU-bound PIL image resizing to threadpool to avoid blocking event loop
            await asyncio.to_thread(downscale_image, saved_cover_path, config.saved_max_width)
    ...
    if embed:
        ...
        if config.embed_max_width > 0:
            # ⚡ Bolt: Offload CPU-bound PIL image resizing to threadpool to avoid blocking event loop
            await asyncio.to_thread(downscale_image, embed_cover_path, config.embed_max_width)
